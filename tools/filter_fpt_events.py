...
def signature(txt):
    ...
    if not segs:
        return None
    return (seq.group(1) if seq else "", pdt[-1] if pdt else "", segs[-1], len(segs))
    ...
    with ThreadPoolExecutor(max_workers=10) as pool:
        ...
        for future in as_completed(futures):
            ...
            if result:
                ...
                # For standard su-kienXX, LIVECDN direct media is preferred
                # because it has proven audio compatibility with NM7.
                if name not in live or source == "LIVECDN":
                    live[name] = (name, verified_url)
